...
Feature: Test the permissions in UDB3
  ...
  Background:
    ...
    And I keep the value of the JSON response at "id" as "placeId_citadel"
    And I set the JSON request payload from "events/rondleiding-citadel.json"
    And I send a POST request to "/imports/events/"
    And I keep the value of the JSON response at "id" as "guidedTourEventId"
    And I wait for the event with url "/events/%{guidedTourEventId}" to be indexed
  ...
  Scenario: update place WITH permission - owner
    Given I am authorized as JWT provider user "invoerder_lgm"
     When I set the JSON request payload to:
        ...
        """
      And I send a PUT request to "/places/%{placeId_molenhuis}/description/nl"
  ...
  Scenario: update place WITH permission - validator_pvb
    Given I am authorized as JWT provider user "validator_pvb"
     When I set the JSON request payload to:
        """
  ...
  Scenario: update place WITHOUT permission - invoerder_dfm
    Given I am authorized as JWT provider user "invoerder_dfm"
     ...
     When I set the JSON request payload to:
        ...
        """
      And I send a PUT request to "/places/%{placeId_molenhuis}/name/nl"
  ...
  Scenario: update place WITHOUT permission - invoerder_gbm
    Given I am authorized as JWT provider user "invoerder_gbm"
      And I send a GET request to "/places/%{placeId_molenhuis}"
      And I keep the value of the JSON response at "name/nl" as "jsonld_name_nl_place_molenhuis"
     When I set the JSON request payload to:
        """
  ...
  Scenario: update event WITH permission - owner
    ...
    When I set the JSON request payload to:
      ...
      """
      And I send a PUT request to "/events/%{guidedTourEventId}/description/nl"
    Then the response status should be "204"
      And I send a GET request to "/events/%{guidedTourEventId}"
    Then the response status should be "200"
      And the JSON response at "description/nl" should be:
      """
  ...
  Scenario: update event WITHOUT permission - invoerder_lgm
    ...
    When I set the JSON request payload to:
      ...
      """
      And I send a PUT request to "/events/%{guidedTourEventId}/name/nl"
    Then the response status should be "403"
      And I send a GET request to "/events/%{guidedTourEventId}"
    Then the response status should be "200"
  ...
  Scenario: add private label to event WITH permission - validator_diest
    Given I am authorized as JWT provider user "validator_diest"
    When I send a PUT request to "/events/%{guidedTourEventId}/labels/private-diest"
    Then the response status should be "204"
    When I send a GET request to "/events/%{guidedTourEventId}"
  ...
  Scenario: add private label to place WITH permission - validator_diest
    Given I am authorized as JWT provider user "validator_diest"
    When I send a PUT request to "/places/%{placeId_citadel}/labels/private-diest"
    Then the response status should be "204"
   When I send a GET request to "/places/%{placeId_citadel}"
   Then the JSON response at "labels" should be:
   """
   ["private-diest"]
   """
  ...
  Scenario: add private label to place WITHOUT permission - validator_scherpenheuvel
    Given I am authorized as JWT provider user "validator_scherpenheuvel"
    When I send a PUT request to "/places/%{placeId_molenhuis}/labels/private-diest"
    Then the response status should be "403"
