...
Feature: Test the Search API v3 advanced queries on offers
  ...
  Scenario: Search for birthdate ranges using an advanced query
    ...
    Then the JSON response at "totalItems" should be 0
    When I send a GET request to "/events" with parameters:
      | q | id:(%{eventId2020} OR %{eventId2022}) AND birthdateRange:2020-01-01..2020-12-31 |
    Then the JSON response at "totalItems" should be 1
    And the JSON response should include:
    """
    %{eventId2020}
    """
    When I send a GET request to "/events" with parameters:
      | q | id:(%{eventId2020} OR %{eventId2022}) AND birthdateRange:(2020-01-01..2020-12-31 OR 2022-06-30..2022-12-31) |
    Then the JSON response at "totalItems" should be 2
    And the JSON response should include:
    """
    %{eventId2020}
  ...
  Scenario: Search for languages using an advanced query
    ...
    And I am using the Search API v3 base URL
    When I send a GET request to "/offers" with parameters:
      | q | id:(%{placeId} OR %{eventId}) AND languages:nl |
    Then the JSON response at "totalItems" should be 0
    When I send a GET request to "/offers" with parameters:
