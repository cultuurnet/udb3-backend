...
final class CalendarNormalizer implements NormalizerInterface
{
    ...
    public function normalize($calendar, $format = null, array $context = []): array
    {
        ...
        if ($calendar instanceof CalendarWithAdjustedOpeningHours && !$calendar->getAdjustedOpeningHours()->isEmpty()) {
            ...
            $data['openingHoursAdjusted'] = array_map(
                fn (\CultuurNet\UDB3\Model\ValueObject\Calendar\AdjustedOpeningHours $aoh) => $adjustedOpeningHoursNormalizer->normalize($aoh),
     ...
     * - For a periodic or permanent calendar this is always available
     * - If there are sub events the top level booking availability type is derived from the sub events
     * In both cases we preserve the capacity from the stored booking availability.
     */
