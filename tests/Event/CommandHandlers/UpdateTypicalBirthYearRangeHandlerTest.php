...
final class UpdateTypicalBirthYearRangeHandlerTest extends CommandHandlerScenarioTestCase
{
    protected function createCommandHandler(EventStore $eventStore, EventBus $eventBus): CommandHandler
    {
        return new UpdateTypicalBirthYearRangeHandler(new EventRepository($eventStore, $eventBus));
    }

    /**
     * @test
     */
    public function it_handles_updating_typical_birth_year_range(): void
    {
        ...
        $this->scenario
            ...
            ->when(new UpdateTypicalBirthYearRange($eventId, $birthYearRange))
     ...
     * @test
     */
    public function it_replaces_existing_typical_birth_year_range(): void
    {
        $eventId = '40021958-0ad8-46bd-8528-3ac3686818a1';
        ...
        $this->scenario
            ->withAggregateId($eventId)
            ->given([$this->getEventCreated($eventId), new TypicalBirthYearRangeUpdated($eventId, $original)])
            ->when(new UpdateTypicalBirthYearRange($eventId, $updated))
    ...
    public function it_ignores_updating_when_unchanged(): void
    {
        ...
        $this->scenario
            ->withAggregateId($eventId)
            ->given([$this->getEventCreated($eventId), new TypicalBirthYearRangeUpdated($eventId, $birthYearRange)])
            ->when(new UpdateTypicalBirthYearRange($eventId, $birthYearRange))
            ->then([]);
    }
